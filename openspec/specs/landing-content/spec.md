...
## Purpose
AI-citable, structured landing page content replacing the previous ~87-word visual-only page.
...
All landing content MUST be present in the raw HTML source (not dependent on JavaScript execution). Content SHALL be rendered by build-time prerender or `<noscript>` fallback.
