...
The system SHALL define a semantic typography scale (`--font-display`, `--font-heading`, `--font-body`, `--font-label`, `--font-caption`) using Cabinet Grotesk for display/headings and Satoshi for body.

#### Scenario: Fonts render correctly
- GIVEN the fonts are self-hosted via `@font-face` from Fontshare
