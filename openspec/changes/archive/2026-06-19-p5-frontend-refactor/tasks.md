...
## PR #1 - Slice A: Auth (targets `feat/p5-frontend-refactor`) ✅ COMPLETED

- [x] 1.1 **Create `web/src/auth/auth-store.ts`** - Zustand store: `user`, `status` (unknown/loading/authenticated/unauthenticated), `restoreSession()`, `login()`, `register()`, `logout()`, `clearSession()`. ~40 LOC. Deps: none. Verify: `pnpm lint && pnpm -F web test -- --run src/auth/`
- [x] 1.2 **Rewrite `AuthProvider.tsx`** - remove Supabase SDK, read from Zustand store, expose same `useAuth()` minus `isAdmin`/`linkIdentity`/`unlinkIdentity`. ~90 LOC. Deps: 1.1. Verify: AuthProvider tests pass.
- [x] 1.3 **Modify `hooks/useAuth.ts`** - re-export store types, drop Supabase imports. ~5 LOC. Deps: 1.2. Verify: `pnpm tsc --noEmit -p web/tsconfig.json`
- [x] 1.4 **Rewrite `AuthCallbackPage.tsx`** - call `authStore.restoreSession()` (GET /api/auth/me) after OAuth redirect. ~15 LOC. Deps: 1.1. Verify: callback renders without Supabase.
- [x] 1.5 **Update `SignInForm.tsx`** - replace `supabase.signInWithPassword` with `authStore.login()`. ~10 LOC. Deps: 1.1. Verify: SignInForm tests pass.
- [x] 1.6 **Update `SignUpForm.tsx`** - replace `supabase.signUp` with `authStore.register()`. ~10 LOC. Deps: 1.1. Verify: SignUpForm tests pass.
- [x] 1.7 **Update `ProtectedRoute.tsx`** - read status from Zustand store. ~15 LOC. Deps: 1.2. Verify: guard tests pass.
- [x] 1.8 **Update `PublicAuthRoute.tsx`** - read status from Zustand store. ~10 LOC. Deps: 1.2. Verify: guard tests pass.
- [x] 1.9 **Update `features/auth/index.ts`** - export store. ~2 LOC. Deps: 1.1. Verify: imports resolve.
- [x] 1.10 **Update test files** - AuthProvider, ProtectedRoute, SignInForm, SignUpForm mocks from AuthClientLike → Zustand mocks. ~60 LOC. Deps: 1.2-1.8. Verify: `pnpm -F web test`
...
## PR #3 - Slice C: Admin Cleanup (targets PR #2 branch) ✅ COMPLETED

- [x] 3.1 **Delete `AdminRoute.tsx`** - remove file. Deps: PR #2. Verify: no imports remain.
- [x] 3.2 **Delete `AdminRoute.test.tsx`** - remove test. Deps: 3.1. Verify: `pnpm test`
- [x] 3.3 **Modify `features/auth/index.ts`** - remove `AdminRoute` export. ~2 LOC. Deps: 3.1. Verify: index compiles.
- [x] 3.4 **Modify `AppRouter.tsx`** - remove `AdminRoute` import + `/app/admin` route block. ~10 LOC. Deps: 3.1. Verify: `pnpm lint && pnpm test`
